...
export function exportToSVG(svgString: string, filename: string = 'worksheet.svg'): void {
...
/** 用 dataURL 下载 PNG */
export function downloadPNG(dataUrl: string, filename: string = 'worksheet.png'): void {
  const link = document.createElement('a');
  link.href = dataUrl;
  link.download = filename;
  document.body.appendChild(link);
