...
export function NumberContentCard({
  label,
  content,
  placeholder,
  onEdit,
  ...
  return (
    <PanelCard>
      ...
      <button
        type="button"
        onClick={onEdit}
        className="border-input bg-background press mt-2 mb-1 flex max-h-24 w-full flex-col gap-1 overflow-hidden rounded-lg border px-3 py-2 text-left shadow-1 transition-colors hover:bg-muted/60"
      >
        <div className="text-muted-foreground text-xs">点击编辑</div>
        <pre className="text-foreground line-clamp-5 overflow-hidden text-sm leading-6 whitespace-pre-wrap">
